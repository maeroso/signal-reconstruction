...
class FISTA(GenericAlgorithm):
    ...
    def run(self) -> None:
        ...
        lambda_value = numpy.multiply(
            numpy.max(numpy.absolute(numpy.matmul(self.global_data.get_transpose_h(), self.signal_array))), 0.10)
        threshold = numpy.absolute(numpy.divide(lambda_value, self.global_data.c))
        f_next = 0
        loop_counter = 0
        loop_maximum = 20
        error = 0
        ...
        for counter in range(loop_maximum):
            ResourceAccess.request_resource(
                ResourceRequest(minimum_free_memory=2, identification=self.ident, maximum_cpu_load=85,
                                thread_event=self.free_resource_event))

            # TODO esse método pesa 2 GB de RAM, deve ser quebrado em pedaçoes menores para debbug e melhoramentos
            f_next = numpy.add(y_old, numpy.matmul(
                numpy.multiply(self.global_data.get_transpose_h(), numpy.divide(1, self.global_data.c)),
                numpy.subtract(self.signal_array, numpy.matmul(self.global_data.H, y_old))))
            index = 0

            ResourceAccess.request_resource(
                ResourceRequest(identification=self.ident, thread_event=self.free_resource_event, maximum_cpu_load=70))
            ...
            alfa_next = numpy.divide(
                numpy.add(1, numpy.sqrt(numpy.add(1, numpy.multiply(4, numpy.power(alfa_old, 2))))), 2)
            y_next = numpy.add(f_next, numpy.multiply(numpy.divide(numpy.subtract(alfa_old, 1), alfa_next),
        ...
        # TODO send image to the backend
