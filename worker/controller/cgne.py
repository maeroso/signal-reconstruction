...
class CGNE(GenericAlgorithm):
    ...
    def run(self) -> None:
        ...
        r_old = numpy.subtract(self.signal_array, numpy.matmul(self.global_data.H, f_old))
        p_old = numpy.matmul(self.global_data.get_transpose_h(), r_old)

        ResourceRequest.request_resource(
            ResourceRequest(maximum_cpu_load=60, identification=self.ident, thread_event=self.free_resource_event))
        ...
        for counter in range(loop_maximum):
            ...
            p_next = numpy.add(numpy.matmul(self.global_data.get_transpose_h(), r_next), numpy.multiply(p_old, beta))
